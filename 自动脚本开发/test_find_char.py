...
e = ss.get(title, {})
tx1, ty1, tx2, ty2 = e["template_rect"]  # 模板区域
sr = e.get("search_region", [0, 0, w, int(h * 0.9)])  # 搜索范围
MATCH_THRESHOLD = 0.7

print(f"截图: {w}x{h}")
print(f"模板区域: ({tx1},{ty1})-({tx2},{ty2})")
print(f"搜索范围: ({sr[0]},{sr[1]})-({sr[2]},{sr[3]})")

# ---- 跟 find_character 完全一致 ----
template_bgr = img[ty1:ty2, tx1:tx2]
sx1, sy1, sx2, sy2 = sr
th, tw = template_bgr.shape[:2]
print(f"模板尺寸: {tw}x{th}  ROI: ({sx1},{sy1})-({sx2},{sy2}) {sx2-sx1}x{sy2-sy1}")

roi = img[sy1:sy2, sx1:sx2]
if roi.shape[0] < th or roi.shape[1] < tw:
    print(f"ERROR: ROI 小于模板!")
else:
    ...
    print(f"最佳匹配: conf={max_val:.3f} pos=({cx},{cy}) {'✓' if found else '✗ 未找到'}")

    # 标注
    out = img.copy()
    cv2.rectangle(out, (sr[0], sr[1]), (sr[2], sr[3]), (255, 0, 0), 2)
    cv2.line(out, (0, sr[3]), (w, sr[3]), (255, 255, 0), 2)
    cv2.rectangle(out, (tx1, ty1), (tx2, ty2), (0, 255, 0), 2)
    cv2.putText(out, "template", (tx1, ty1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 255, 0), 1)
    cv2.circle(out, (cx, cy), 14, (0, 0, 255), 3)
    cv2.putText(out, f"角色({cx},{cy}) conf={max_val:.2f}", (cx + 16, cy),
                cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)
    cv2.putText(out, f"search_y={sr[3]}", (sr[0], sr[3] - 5),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)

    pp = Path(__file__).resolve().parent / "test_output" / "find_char_result.png"
